<?php
session_start();
$pass = 'barbershop'; // Пароль для входа в Админку
...
<a href="admin.php?mode=16">Заявки</a>
<a href="admin.php?mode=11">Отзывы</a>
<a href="admin.php?mode=12">Видео</a>
<a href="admin.php?mode=13">Новости</a>
<a href="admin.php?mode=15">Барберы</a>
...
// Замена картинки
	if ($_GET['mode'] == '2') {
		...
		if (move_uploaded_file($_FILES['userfile']['tmp_name'], $imgname)) {
			echo "<br><br><center>Файл был успешно загружен.<p><a href='admin.php'>Вернуться к списку картинок</a><p>Что бы увидеть изменения на сайте, обновите страницу в гугл хроме с зажатей клавишей shift (не эту (это админка), а страницу Вашего сайта)";
		}
		;
	}
	;
...
// Редактирование текстового фрагмента
	if ($_GET['mode'] == '4') {
		...
		echo "<br><br><center>Текст был успешно изменен.<p><a href='admin.php?mode=0'>Вернуться к списку текстов</a><p>Что бы увидеть изменения на сайте, обновите страницу (не эту (это админка), а страницу Вашего сайта)";
	}
	;
...
// Форма для HTML кода
	if ($_GET['mode'] == '5') {
		...
		echo ('<div style="margin: 0 auto; text-align: center;"><form method="POST" action="admin.php?mode=6"><br><br><h2>Редактирование HTML кода</h2><br><br><textarea name="mytext" style="width: 90%; height: 500px;">' . $template . '</textarea><br><input style="width: 300px;
    padding-top: 19px;
    padding-bottom: 22px;
    background-color: #005bff;
    box-shadow: 0 20px 18px -10px rgba(0, 91, 255, 0.43);
...
// Форма для HTML кода
	if ($_GET['mode'] == '9') {
		...
		<form method="POST" action="admin.php?mode=10&fl=' . $_GET['fl'] . '">
		<br><br>
		<h2>Редактирование кода</h2>
		<br><br>
		<textarea name="mytext" style="width: 90%; height: 500px;">' . $template . '</textarea>
		<br>
		<input style="
			width: 300px;
			padding-top: 19px;
			...
			color: white;
			margin-top: 20px;" 
		type="submit" value="Заменить текст" title="Заменить текст">
		</form>
		</div>');
	}
	;
...
//Редактирование всего HTML кода
	...
	if ($_GET['mode'] == '11') {
		include ('form_feedback.php');
	}
	;

	if ($_GET['mode'] == '12') {
		include ('form_video.php');
	}
	;

	if ($_GET['mode'] == '15') {
		include ('form_barbers.php');
	}
	;
...
// Помощь
	if (!isset($_GET['mode'])) {
		echo ('<div id="help"><p><br><h2>Вы находитесь в панели управления сайтом.</h2></div>');
	}
	;
